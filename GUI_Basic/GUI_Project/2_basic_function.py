...
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요", filetypes=(
        ("PNG파일", "*.png"), ("모든파일", "*.*")), initialdir=r"C:\Users\Admin\Desktop\python_project\pyGame")  # 최초의 사용자가 지정한경로를 보여줌
...
btn_add_file = Button(file_frame, text="파일 추가", width=12,
                      padx=5, pady=5, command=add_file)
btn_add_file.pack(side="left")
btn_del_file = Button(file_frame, text="선택 삭제", width=12,
                      padx=5, pady=5, command=del_file)
...
list_file = Listbox(list_frame, selectmode="extended",
                    height=15, yscrollcommand=scroll_bar.set)
list_file.pack(side="left", fill="both", expand=True)
scroll_bar.config(command=list_file.yview)
...
btn_dest_path = Button(path_frame, text="찾아보기", width=10,
                       padx=5, pady=5, command=browse_dest_path)
...
# 가로넓이 레이블
label_width = Label(frame_option, text="가로 넓이", width=8)
...
# 파일 포맷 옵션 레이블
label_format = Label(frame_option, text="가로 넓이", width=8)
